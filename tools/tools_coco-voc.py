...
def parse_args():
    ...
    parser.add_argument('--voc_src', help='the source dir of voc')
    parser.add_argument(
...
def mv_voc2coco(voc_src,cocovoc):
    """
    把voc的train和val的图片放入 coco 的train2017和val2017
    ...
    os.system('python tools/dataset_converters/pascal_voc.py '+voc_src+' '+'-o '+cocovoc+'/annotations/  '+'--out-format  coco')
...
def remap_voc_coco_cls(cocovoc):
    """
    voc的类别改成coco的类别
...
def main():
    ...
    #把voc的图片放入coco
    mv_voc2coco(args.voc_src, args.cocovoc)

    # coco添加类sofa tvmonitor
    coco_add_cls(args.cocovoc)

    # #voc的类别改成coco的类别
    remap_voc_coco_cls(args.cocovoc)    
